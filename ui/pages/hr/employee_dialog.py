...
class _SubTable(QWidget):
    ...
    def __init__(self, title, columns, load_fn, add_fn, delete_fn, parent=None):
    ...
    def refresh(self):
        ...
        for r, (values, entity_id) in enumerate(rows):
            ...
            self.table.setCellWidget(r, len(values), cell_widget(del_btn))


class _AddQualificationDialog(BaseDialog):
    def __init__(self, hr_service, employee_id, parent=None):
        super().__init__("إضافة مؤهل علمي", parent, min_width=440)
    ...
    def on_save(self):
        ...
        try:
            self.hr_service.add_qualification(
                self.employee_id, degree, institution, self.year.widget.value() or None)
            self.accept()
        except Exception as e:
            logger.error(f"Add qualification failed: {e}")
            Toast.error(self, "تعذّر حفظ المؤهل")


class _AddExperienceDialog(BaseDialog):
    def __init__(self, hr_service, employee_id, parent=None):
        super().__init__("إضافة خبرة عملية", parent, min_width=480)
        self.hr_service = hr_service
        self.employee_id = employee_id
    ...
    def on_save(self):
        ...
        try:
            self.hr_service.add_experience(
                self.employee_id, job, company,
                self.start.widget.date().toPython(),
                self.end.widget.date().toPython(),
...
class _AddEvaluationDialog(BaseDialog):
    def __init__(self, hr_service, employee_id, parent=None):
        super().__init__("إضافة تقييم", parent, min_width=460)
        self.hr_service = hr_service
        self.employee_id = employee_id
        eval_date = QDateEdit()
    ...
    def on_save(self):
        try:
            self.hr_service.add_evaluation(
                self.employee_id,
                self.date.widget.date().toPython(),
                self.score.widget.value(),
                self.notes.text().strip() or None)
            self.accept()
        except Exception as e:
            logger.error(f"Add evaluation failed: {e}")
...
class EmployeeDialog(BaseDialog):
    ...
    def _build_records_tab(self) -> QWidget:
        ...
        layout.addWidget(_SubTable(
            ...
            lambda: [
                ([q.degree, q.institution, q.year_obtained or "-"], q.id)
